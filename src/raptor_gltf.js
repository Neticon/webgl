...
const camera = new THREE.PerspectiveCamera(45, W / H, .01)
...
function init() {
  ...
  new THREE.GLTFLoader().load('./assets/animation/raptor_gltf/scene.gltf', onLoad, console.log, console.log)
}
...
function onLoad(object) {
  ...
  for (const animation of animations) {
    ...
    // actions[animation.name].getClip().optimize()
    // actions[clip.name].setDuration(10)
  }
  scene.add(object.scene)
  fitCamera(object.scene)
  setGui()
}
...
function addActionControl(action) {
  ...
  folder.add(API, 'time =', 0, clip.duration, 0.1).listen();
