const _ = {
  select: (arr, fn) => arr.filter(fn),
  negate: fn => (...args) =>!fn(...args),
  reject: function(arr, fn){return arr.filter(this.negate(fn))},
  detect: (arr, fn) => arr.find(fn),
  include: (arr, probe) => arr.includes(probe),
  indexOf: (arr, probe) => arr.indexOf(probe),
...
export function MazeGenerator(rows, cols) {
  ...
  this.solve = function() {
    ...
    while(openSet.length > 0) {
      ...
      openSet.splice(_.indexOf(openSet, searchCell),1);
      searchCell = null;
